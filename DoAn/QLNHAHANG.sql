...
INSERT INTO DatBan(MaBan, MaNV, ThoiGian, MaKhachHang, SoLuongKhach, TinhTrang)
VALUES 
('2', '4', '10:56:44', '1', 10, N'CHƯA THANH TOÁN'),
('4', '3', '14:43:22', '8', 20, N'ĐÃ THANH TOÁN'),
('5', '3', '12:55:12', '3', 20, N'CHƯA THANH TOÁN'),
('2', '1', '17:56:32', '7', 10, N'ĐÃ THANH TOÁN')
...
(N'NHÃN', 'KG', 4),
(N'XOÀI', 'KG', 6),
(N'BỘT RAU CÂU', 'BỊCH', 7),
(N'BỘT CỐT DỪA', 'BỊCH', 10),
(N'THẠCH DỪA', 'LON', 4),
(N'PHỞ', 'KG', 6)
...
--trigger cập nhật trạng thái bàn
CREATE TRIGGER Trigger_CapNhatTrangThaiBan
ON DatBan
AFTER INSERT, UPDATE
AS
BEGIN
    UPDATE Ban
    SET TrangThai = N'ĐỦ CHỖ'
    FROM Ban
    INNER JOIN inserted ON Ban.MaBan = inserted.MaBan;

    UPDATE Ban
    SET TrangThai = N'CÒN TRỐNG'
    FROM Ban
    LEFT JOIN DatBan ON Ban.MaBan = DatBan.MaBan
    WHERE DatBan.MaBan IS NULL;
END;
